...
public:
    nanbeige_npu_sequence(){}

    /// \brief Constructor
    /// \param config the configuration
    /// \param MAX_L the max length
    nanbeige_npu_sequence(LM_Config config, uint32_t MAX_L);
    ~nanbeige_npu_sequence();
    ...
    /// \param MAX_L the max length
    void gen_mha_engine_seq(npu_sequence* seq, const uint32_t L_begin, const uint32_t L_end, const uint32_t MAX_L);
