...
qwen2vl_image_t Qwen2VL::load_image(const std::string& filename) {
    ...
    if (this->image_pre_resize > 0) {
        ...
        if (decoded.height > max_height) {
            ...
            header_print_r("OFLM", "Qwen2VL resizing image from (" + std::to_string(decoded.width) + ", " + std::to_string(decoded.height) + ") to (" + std::to_string(target_width) + ", " + std::to_string(target_height) + ")\n");
