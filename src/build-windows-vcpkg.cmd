...
if not exist "..\third_party\tokenizers-cpp\CMakeLists.txt" (
    echo [oflm] third_party\tokenizers-cpp is missing: clone it first ^(see the header^)
    exit /b 1
)
...
cmake -S . -B build -G "Visual Studio 17 2022" -A x64 -DCMAKE_GENERATOR_INSTANCE="%VS_INSTANCE:\=/%" ^
  ...
  -DOFLM_VERSION=0.1.0 -DNPU_VERSION=32.0.203.304 -DCMAKE_BUILD_TYPE=Release
if errorlevel 1 (
...
cmake --build build --config Release --parallel 8 --target oflm
if errorlevel 1 exit /b 1
...
echo [oflm] OK -^> out\oflm.exe   (set OFLM_MODEL_PATH=%%USERPROFILE%%\.oflm from a non-interactive shell)
exit /b 0
